...
export class VideoService extends Service implements IVideoService {
    ...
    public async downloadMedia(url: string): Promise<string> {
        ...
        try {
            ...
            elizaLogger.error("Error downloading media:", error);
    ...
    async fetchVideoInfo(url: string): Promise<any> {
        if (url.endsWith(".mp4") || url.includes(".mp4?")) {
            try {
                ...
                elizaLogger.error("Error downloading MP4 file:", error);
        ...
        try {
            ...
            elizaLogger.error("Error fetching video info:", error);
    ...
    private async getTranscript(
        ...
        try {
            ...
            elizaLogger.error("Error in getTranscript:", error);
            throw error;
        }
    }
    ...
    private parseCaption(captionContent: string): string {
        ...
        try {
            ...
            if (jsonContent.events) {
                ...
                elizaLogger.error("Unexpected caption format:", jsonContent);
                return "Error: Unable to parse captions";
            }
        } catch (error) {
            elizaLogger.error("Error parsing caption:", error);
            return "Error: Unable to parse captions";
        }
    }
    ...
    private async convertMp4ToMp3(
        ...
        return new Promise((resolve, reject) => {
            ffmpeg(inputPath)
                ...
                .on("error", (err) => {
                    elizaLogger.error("Error converting to MP3:", err);
                    reject(err);
                })
                .run();
        });
    ...
    private async downloadAudio(
        ...
        try {
            ...
            elizaLogger.error("Error downloading audio:", error);
