...
describe("Metering", () => {
    ...
    it("should clear existing flush timer when starting auto flush", () => {
        ...
        const startAutoFlushMethod =
            Object.getPrototypeOf(meteringInstance).constructor.prototype
                .startAutoFlush;
    ...
    it("should handle errors during auto flush interval execution", () => {
        ...
        const flushMock = vi
            .fn()
            .mockRejectedValue(new Error("Auto flush error"));
        ...
        meteringInstance.track({
            specversion: "1.0",
            type: "test-event",
            id: stringToUuid("auto-flush-test"),
            time: new Date().toISOString(),
            source: "test-source",
    ...
    it("should handle flush errors after reaching batch size", async () => {
        ...
        // Mock fetch to fail
        const fetchErrorMock = vi
            .fn()
            .mockRejectedValue(
                new Error("Network error on batch size trigger")
            );
    ...
    it("should return success and empty eventIds when flushing with empty queue", async () => {
        ...
        // Mock elizaLogger.debug directly by importing and mocking the module
        const elizaLogger = (await import("../src/logger")).default;
        const debugSpy = vi
            .spyOn(elizaLogger, "debug")
            .mockImplementation(() => {});
    ...
    it("should log events locally when no API key is provided", async () => {
        ...
        // Mock elizaLogger for capturing debug logs
        const elizaLogger = (await import("../src/logger")).default;
        const debugSpy = vi
            .spyOn(elizaLogger, "debug")
            .mockImplementation(() => {});
