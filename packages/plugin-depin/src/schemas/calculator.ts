...
export const CalculatorToolSchema = {
    ...
    parameters: z.object({
        ...
        operand2: z
            .number()
            .optional()
            .describe("The second operand (not required for sqrt operation)"),
    }),
    execute: async (args: {
...
class CalculatorTool {
    ...
    private validateOperands(
        ...
        if (operation !== "sqrt" && operand2 === undefined) {
            throw new Error(
                `Second operand is required for operation: ${operation}`
            );
        }
    ...
    private async withErrorHandling<T>(
        ...
        try {
            ...
            elizaLogger.error(`Error executing ${operation} tool`, error);
