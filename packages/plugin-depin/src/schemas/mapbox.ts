...
const GeocodeFeatureSchema = z.object({
    ...
    properties: z.object({
        ...
        full_address: z
            .string()
            .optional()
            .describe("Complete formatted address"),
        accuracy: z
            .string()
            .optional()
...
export const GetCoordinatesToolSchema = {
    ...
    execute: async (args: GeocodeParams) => {
        try {
            ...
            elizaLogger.error("Error executing get_coordinates tool", error);
            return `Error executing get_coordinates tool`;
        }
    },
};

export const GetLocationFromCoordinatesToolSchema = {
    ...
    execute: async (args: ReverseGeocodeParams) => {
        try {
            ...
            elizaLogger.error(
                "Error executing get_location_from_coordinates tool",
                error
            );
...
export const GetDirectionsToolSchema = {
    name: "get_directions",
    description:
    ...
    execute: async (args: DirectionsParams) => {
        try {
            ...
            return {
                summary: {
                    distance:
                        (directions.routes[0].distance / 1000).toFixed(1) +
                        " km",
                    duration:
                        Math.round(directions.routes[0].duration / 60) +
                        " minutes",
                    origin: args.origin,
            ...
            elizaLogger.error("Error executing get_directions tool", error);
...
export class MapboxTool {
    ...
    constructor() {
        this.baseUrl = MAPBOX_GEOCODING_URL;
    ...
    async getReverseGeocodingData(
        ...
        try {
            ...
            throw new Error(
                `Failed to get reverse geocoding data: ${error.message}`
            );
        }
    }
    ...
    async getDirections(params: {
        ...
        try {
            ...
            if (error.name === "ZodError") {
                throw new Error(
                    `Invalid directions response format: ${error.message}`
                );
            }
    ...
    async getGeocodingData(
        ...
        try {
            const validatedParams =
                GetCoordinatesToolSchema.parameters.parse(params);

            const queryParams = new URLSearchParams({
                q: validatedParams.location,
                access_token: process.env.MAPBOX_ACCESS_TOKEN!,
                limit: validatedParams.limit?.toString() || "1",
                ...(validatedParams.country && {
    ...
    async getRawData(
        ...
        if ("location" in params) {
            ...
            const { originCoords, destCoords } =
                await this.extractCoordinates(params);
    ...
    private async extractCoordinates(params: DirectionsParams): Promise<{
