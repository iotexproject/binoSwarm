...
export const GetMetricsToolSchema = {
    ...
    parameters: z.object({
        isLatest: z
            ...
            .default(true)
            .describe(
                "Whether to fetch only the latest metrics or historical data"
            ),
    }),
    execute: async (args: { isLatest: boolean }) => {
        try {
            const tool = new DePINScanMetricsTool();
            const metricsData = await tool.getRawData({
            ...
            elizaLogger.error("Error executing get_depin_metrics tool", error);
            return `Error executing get_depin_metrics tool`;
        }
    },
};

export const GetProjectsToolSchema: {
    name: string;
    description: string;
    parameters: any;
    ...
    execute: async (args: z.infer<typeof GetProjectsToolSchema.parameters>) => {
        try {
            ...
            elizaLogger.error("Error executing get_depin_projects tool", error);
...
const ProjectFilters = {
    ...
    byMinNumber:
        (
            value: number,
            field: keyof DepinScanProjectResponse[number]
        ): FilterPredicate =>
        (project) => {
    ...
    byMaxNumber:
        ...
        (project) => {
            ...
            return (
                ...
                Number(projectValue) <= value
            );
        },

    hasToken: (): FilterPredicate => (project) =>
        Boolean(project.token?.length),
};
...
const filterConfigs: Record<string, FilterConfig> = {
    ...
    minMarketCap: {
        ...
        createFilter: (args) =>
            ProjectFilters.byMinNumber(args.minMarketCap!, "market_cap"),
    },
    minDevices: {
        condition: (args) => args.minDevices !== undefined,
        createFilter: (args) =>
            ProjectFilters.byMinNumber(args.minDevices!, "total_devices"),
    },
    requireToken: {
    ...
    minTokenPrice: {
        ...
        createFilter: (args) =>
            ProjectFilters.byMinNumber(args.minTokenPrice!, "token_price"),
    },
    minDailyEarnings: {
        condition: (args) => args.minDailyEarnings !== undefined,
        createFilter: (args) =>
            ProjectFilters.byMinNumber(
                args.minDailyEarnings!,
    ...
    maxDaysToBreakeven: {
        ...
        createFilter: (args) =>
            ProjectFilters.byMaxNumber(
                args.maxDaysToBreakeven!,
                "days_to_breakeven"
            ),
    },
};
...
export class DePINScanMetricsTool {
    constructor() {}

    async getRawData(
        params: DepinScanMetricsParams
    ): Promise<DepinScanMetricsResponse> {
...
export class DePINScanProjectsTool {
    constructor() {}
...
function numberOrZeroIfNanToString(
